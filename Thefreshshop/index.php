      ...
      <div class="item">
        ...
        <div class="all-item-list row row-cols-5">
          <?php
              ...
              ORDER BY TSL  DESC LIMIT 0,4";

              $sql_hot_resutl = executeResutl($sql_hot);
              foreach ($sql_hot_resutl as $sphot) {
                $string_temp1 = "'".$sphot['MSHH']."'";
                echo'
                <div class="col" id="item_card sp-hot" >
                <img id="hot-icon" src="./img/icon-hot.png" alt="">
                <a href="chitietsanpham.php?MSHH='.$sphot['MSHH'].'"><img src="./img/'.$sphot['MaLoaiHang'].'/'.$sphot['MSHH'].'_1.jpg"></a>
                
                <a href="chitietsanpham.php?MSHH='.$sphot['MSHH'].'"><strong class="title_name_item" >
                '.$sphot['TenHH'].'
                </strong></a>
                <p>Giá: <span class="price_item">'.$sphot['Gia'].'</span> VNĐ</p>
                <div class="buy_item">
                  <p>Sô Lượng: '.$sphot['SoLuongHang'].'</p>
                  <button onclick="addToCart('.$string_temp1.')">Thêm vào giỏ</button>
                </div>
          </div>
          ...
          if(!empty($_REQUEST['search'])){
            ...
            $sqlSearch = 'where hanghoa.TenHH LIKE "%'. $searchItem.'%";';
         
          }else{
           
          }
          $sql = 'select count(MSHH) as number from hanghoa ' . $sqlSearch;
          ...
          $sql = 'SELECT hanghoa.MaLoaiHang , hanghoa.MSHH,  hanghoa.TenHH, hanghoa.Gia, hanghoa.SoLuongHang, hinhhanghoa.TenHinh FROM hanghoa, hinhhanghoa WHERE hanghoa.MSHH = hinhhanghoa.MSHH and hinhhanghoa.TenHinh LIKE "%_1" limit ' . $index . ',' . $item;
          // Test
          if (!empty($_REQUEST['search'])) {
            $searchItem = $_REQUEST['search'];
            $sql = 'SELECT hanghoa.MaLoaiHang , hanghoa.MSHH,  hanghoa.TenHH, hanghoa.Gia, hanghoa.SoLuongHang, hinhhanghoa.TenHinh FROM hanghoa, hinhhanghoa WHERE hanghoa.MSHH = hinhhanghoa.MSHH and hanghoa.TenHH LIKE "%' . $searchItem . '%" and hinhhanghoa.TenHinh LIKE "%_1" limit ' . $index .',' . $item;
          } else {
          }
        ...
        <ul class="pagination justify-content-center">
          <li class="page-item">
            ...
            if ($page > 1) {
              ...
              $page = $page - 1;
              echo '<a class="page-link list-group-item" href="?search='.$searchItem.'&page=' . $page . '">&laquo;</a>';
            } else {
              echo '<a class="page-link  list-group-item disabled" href="?search='.$searchItem.'&page=' . $page . '">&raquo;</a>';
            }
          ...
          for ($i = 1; $i <= $pages; $i++) {
            ...
            echo '<li class="page-item"><a class="page-link list-group-item" href="?search='.$searchItem.'&page=' . $i . '">' . $i . '</a></li>';
          }
          ?>
          <li class="page-item">
            ...
            if ($page < $pages) {
              ...
              $page = $page + 1;
              echo '<a class="page-link list-group-item" href="?search='.$searchItem.'&page=' . $page . '">&raquo;</a>';
            } else {
              echo '<a class="page-link  list-group-item disabled" href="?search='.$searchItem.'&page=' . $page . '">&laquo;</a>';
            }
